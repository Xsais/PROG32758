...
public class NewUserServlet extends HttpServlet {
    ...
    private String lastName, firstName, group, login, password, preferredCarName, credit, score, logo;

    private String lockCode = null;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (lockCode == null) {
            ...
            lockCode = (String) request.getServletContext().getAttribute("db_lockOut");
        }

        password = request.getParameter("password");

        if (password == lockCode) {

            response.sendRedirect(String.format("./newuser.jsp?err=%d", 2));
            return;
        }
            ...
            if (rs.next()) {
                ...
                if (results.next()) {
                    ...
                    dbConnection.executeUpdate(String.format("UPDATE players SET "
                                    + "`Login`='%s', `Password`='%s',`Preferred_Car_Name`='%s',`Logo`=%s, `Score`=%s," +
                                    " `Credits`=%s "
                                    + "WHERE `Last_Name`='%s' AND `First_Name`='%s' AND `Group`=%s", login, password,
                            preferredCarName, logo, Integer.parseInt(score), Double.parseDouble(credit),
